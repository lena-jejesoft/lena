...
import type { CartesianStyle, ChartCoreLegendMeta, ChartData, ChartStyle, ChartType, OHLCPoint, Scenario } from "@/components/chart/types"
import { getCompatibleChartTypes } from "@/components/chart/registry"
import { analyzeDataQualityExtended } from "@/components/chart/recharts-core/recharts-adapter"
import { ChartLegendPanel } from "@/components/chart/recharts-core/chartTool/chart-legend-panel"
import type { ChartType as LegendPanelChartType, ExtendedDataAnalysisResult } from "@/components/chart/recharts-core/recharts-type"
import { supportsOutliers } from "@/components/chart/chartCore/src/types/chart-type-config"
...
function ToggleSwitch({
  ...
  return (
    ...
    >
      <span
        className={cn(
          "absolute top-[2px] left-[2px] h-[14px] w-[14px] rounded-full bg-background transition-transform",
    ...
    <div className="rounded-md border border-border p-3 space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <Input
          type="color"
          value={displayColor}
          onChange={(event) => setSeriesColor(block.id, seriesId, event.target.value)}
...
function StylePanelContent({
  ...
  const chartStateColorMap = getSeriesDisplayColors(activeBlock, chartState.seriesColors)
  ...
  return (
    <div className="space-y-4">
      ...
      {canConfigureGroupColors && groupedLegendEntries.length > 0 && (
        <section className="space-y-2">
...
function SeriesPanelContent({
  ...
  activeBlock: BlendedChartBlock
  chartState: BlendedChartViewState
  tooltipPayload: any[] | null
  hoveredLabel: string | null
  treemapStats: any
  onBlockStyleChange: (blockId: string, updater: (prev: ChartStyle) => ChartStyle) => void
  ...
  const colorMap = getSeriesDisplayColors(activeBlock, chartState.seriesColors)
  const seriesColors = seriesFields.map((field) => colorMap[field] ?? BASE_PALETTE[0])
...
function Phase3Screen() {
  ...
  const [sideTab, setSideTab] = useState<"series" | "style">("series")
  ...
  const [showDerivedPanel, setShowDerivedPanel] = useState(true)
  const [showSemanticMappingPanel, setShowSemanticMappingPanel] = useState(true)
  const [showAdvancedDataControls, setShowAdvancedDataControls] = useState(false)
  const {
  ...
  const isPanel23Open = !isConfigCollapsed
  const chartHeaderTitleOffset = !isPanel23Open
    ? (!isPanel1Open ? 76 : 40)
    : 0
  ...
  const chartTypeOptionsForControl = useMemo(() => {
    ...
    return [{ value: activeBlock.chartType, label: activeBlock.chartType }, ...CHART_TYPE_OPTIONS]
  }, [activeBlock])
    ...
    <Page direction="horizontal" className="relative h-screen">
      <Panel
        ...
        className={cn(
          ...
          isPanel1Open ? "w-[280px]" : "w-0 border-r-0 opacity-0 pointer-events-none"
        )}
      >
        {isPanel1Open && (
          <>
              ...
              <div className="space-y-2">
                <div className="rounded-sm border border-border p-2">
                  <Button
                    ...
                    onClick={() => setShowHierarchyPanel((prev) => !prev)}
                  >
                    계층 탐색
                    <span>{showHierarchyPanel ? "접기" : "열기"}</span>
                  </Button>
                  {showHierarchyPanel && (
                    <div className="mt-2">
                      <PageHHierarchyPanel
                        companies={hierarchyCompanies}
                        companyId={hierarchyState.companyId}
                        companySearchTerm={hierarchyState.companySearchTerm}
                        metricSearchTerm={hierarchyState.metricSearchTerm}
                        dimensionKey={hierarchyState.dimensionKey}
                        sourceNodes={hierarchySourceNodes}
                        selectedMetricKeys={hierarchyState.selectedMetricKeys}
                        selectedMetrics={hierarchySelectedMetrics}
                        isCompaniesLoading={isHierarchyCompaniesLoading}
                        isMetricsLoading={isHierarchyMetricsLoading}
                        isApplying={isHierarchyApplyLoading}
                        error={hierarchyError}
                        onCompanyIdChange={setHierarchyCompanyId}
                        onCompanySearchTermChange={setHierarchyCompanySearchTerm}
                        onMetricSearchTermChange={setHierarchyMetricSearchTerm}
                        onDimensionKeyChange={setHierarchyDimensionKey}
                        onSearchCompanies={handleHierarchyCompanySearch}
                        onToggleMetric={toggleHierarchyMetricSelection}
                        onRemoveMetric={removeHierarchyMetricSelection}
                        onClearMetrics={clearHierarchyMetricSelections}
                        onApply={handleApplyHierarchyMetrics}
                      />
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-sm border border-border p-2">
                  <Button
                    ...
                    size="sm"
                    className="h-7 w-full justify-between text-xs"
                    onClick={() => setShowDerivedPanel((prev) => !prev)}
                  >
                    파생 지표 계산
                    <span>{showDerivedPanel ? "접기" : "열기"}</span>
                  </Button>
                  {showDerivedPanel && (
                    <div className="mt-2">
                      <PageHDerivedPanel
                        formulaOptions={DERIVED_FORMULA_PRESETS}
                        selectedFormulaId={derivedState.formulaId}
                        selectedFormulaInputCount={derivedFormulaPreset.inputCount}
                        metricOptions={derivedMetricOptions}
                        primaryMetricKey={derivedState.primaryMetricKey}
                        secondaryMetricKey={derivedState.secondaryMetricKey}
                        customName={derivedState.customName}
                        error={derivedError}
                        onFormulaChange={(formulaId) => handleDerivedFormulaChange(formulaId as DerivedFormulaId)}
                        onPrimaryMetricChange={setDerivedPrimaryMetricKey}
                        onSecondaryMetricChange={setDerivedSecondaryMetricKey}
                        onCustomNameChange={setDerivedCustomName}
                      />
                    </div>
                  )}
                </div>
                {blendMessage && (
                  <div className="rounded-sm border border-border bg-secondary/40 p-2 text-[11px] text-muted-foreground">
                    {blendMessage}
                  </div>
                )}
                ...
                <div className="rounded-sm border border-border p-2">
                  <Button
                    ...
                    size="sm"
                    className="h-7 w-full justify-between text-xs"
                    onClick={() => setShowAdvancedDataControls((prev) => !prev)}
                  >
                    고급 데이터 제어
                    <span>{showAdvancedDataControls ? "접기" : "열기"}</span>
                  </Button>

                  {showAdvancedDataControls && (
                    <div className="mt-2 space-y-2">
                      {/* <Label htmlFor="data-search" className="text-xs">데이터 검색창</Label>
                      <Input
                        id="data-search"
                        placeholder="필드 검색..."
                        className="h-8 text-xs"
                      ...
                      <Button
                        ...
                        size="sm"
                        className="h-8 w-full text-xs"
                        onClick={handleBlendSources}
                        disabled={!hasAtLeastOneSource}
                      >
                        {hasBothSources ? (isAppendMode ? "시리즈 병합" : "데이터 결합") : "데이터 표시"}
                      </Button>

                      <div className="space-y-1.5">
                        <div className="rounded-sm border border-border p-2 text-[11px]">
                          <p className="font-medium">데이터 A</p>
                          {uploadedA ? (
                            <div className="mt-1 space-y-0.5 text-muted-foreground">
                              <p className="truncate">{uploadedA.fileName}</p>
                              <p>{formatFileSize(uploadedA.fileSize)} / {uploadedA.rowCount == null ? "행수 미분석" : `${uploadedA.rowCount}행`}</p>
                              <p>{uploadedA.columns.length > 0 ? `${uploadedA.columns.length}개 컬럼` : "컬럼 미분석"}</p>
                            </div>
                          ) : (
                            <p className="mt-1 text-muted-foreground">업로드 전</p>
                          )}
                        </div>

                        <div className="rounded-sm border border-border p-2 text-[11px]">
                          <p className="font-medium">데이터 B</p>
                          {uploadedB ? (
                            <div className="mt-1 space-y-0.5 text-muted-foreground">
                              <p className="truncate">{uploadedB.fileName}</p>
                              <p>{formatFileSize(uploadedB.fileSize)} / {uploadedB.rowCount == null ? "행수 미분석" : `${uploadedB.rowCount}행`}</p>
                              <p>{uploadedB.columns.length > 0 ? `${uploadedB.columns.length}개 컬럼` : "컬럼 미분석"}</p>
                            </div>
                          ) : (
                            <p className="mt-1 text-muted-foreground">업로드 전</p>
                          )}
                      ...
                      {hasBothSources && isDualOhlcOverlayMode && (
                        <div className="rounded-sm border border-border p-2 text-[11px] text-muted-foreground">
                          OHLC 오버레이 모드에서는 X/Y 축 선택을 사용하지 않습니다.
                        </div>
                      )}

                      {!hasBothSources && isSingleOhlcMode && singleSource && (
                        <div className="rounded-sm border border-border p-2 text-[11px] text-muted-foreground">
                          {singleSource.slot} 슬롯이 OHLC 데이터로 인식되어 X/Y 축 선택을 사용하지 않습니다.
                        </div>
                      )}

                      {!hasBothSources && !isSingleOhlcMode && singleAxisUiState && (
                        <PageHUploadAxisPanel
                          title={`단일 데이터 축 선택 (${singleAxisUiState.slot})`}
                          description="X축 1개와 Y축(복수)을 선택합니다. Y를 날짜 헤더(예: 2025/01)로 고르면 날짜를 X축으로 자동 변환합니다."
                          availableXKeys={singleAxisUiState.xOptions}
                          selectedXKey={singleAxisUiState.xKey}
                          xKeyLabelMap={AXIS_X_KEY_LABEL_MAP}
                          availableYKeys={singleAxisUiState.yOptions}
                          selectedYKeys={singleAxisUiState.selectedYKeys}
                          onXChange={(xKey) => handleSingleAxisXChange(singleAxisUiState.slot, xKey)}
                          onYToggle={(yKey) => handleSingleAxisYToggle(singleAxisUiState.slot, yKey)}
                          onYSelectAll={() => handleSingleAxisYSelectAll(singleAxisUiState.slot, singleAxisUiState.yOptions)}
                          onYClear={() => handleSingleAxisYClear(singleAxisUiState.slot)}
                        />
                      )}

                      {hasBothSources && !isDualOhlcOverlayMode && isAppendMode && (
                        <div className="rounded-sm border border-border p-2 text-[11px] text-muted-foreground">
                          {hasWidePeriodBlendPair
                            ? "현재 파일은 기간 헤더 기반 wide 데이터로 인식되어 시리즈 병합(A+B)을 기본값으로 제안합니다."
                            : "시리즈 병합 모드에서는 A/B 행을 그대로 이어 붙여 같은 차트에서 함께 보여줍니다."}
                        </div>
                      )}
                      ...
                      {hasBothSources && !isDualOhlcOverlayMode && isAppendMode && (
                        <PageHUploadAxisPanel
                          title="병합 데이터 축 선택"
                          description="A/B 데이터를 같은 축에서 함께 보여줄 X/Y 축을 선택합니다."
                          availableXKeys={appendXAxisOptions}
                          selectedXKey={appendResolvedXKey}
                          xKeyLabelMap={AXIS_X_KEY_LABEL_MAP}
                          availableYKeys={appendYAxisOptions}
                          selectedYKeys={appendSelectedYKeys}
                          onXChange={handleAppendAxisXChange}
                          onYToggle={handleAppendAxisYToggle}
                          onYSelectAll={handleAppendAxisYSelectAll}
                          onYClear={handleAppendAxisYClear}
                        />
                      )}
                      ...
                      {hasBothSources && !isDualOhlcOverlayMode && !isAppendMode && (
                        <PageHUploadAxisPanel
                          title="결합 데이터 축 선택"
                          description="결합에 사용할 키는 아래에서 별도로 선택하고, 차트 X/Y 축은 여기서 선택합니다."
                          availableXKeys={joinXAxisOptions}
                          selectedXKey={joinResolvedXKey}
                          xKeyLabelMap={AXIS_X_KEY_LABEL_MAP}
                          availableYKeys={joinYAxisOptions}
                          selectedYKeys={joinSelectedYKeys}
                          onXChange={handleJoinAxisXChange}
                          onYToggle={handleJoinAxisYToggle}
                          onYSelectAll={handleJoinAxisYSelectAll}
                          onYClear={handleJoinAxisYClear}
                        />
                      )}
                        ...
                        <div className="space-y-2 rounded-sm border border-border p-2">
                          <div className="flex items-center justify-between gap-2">
                            <Button
                              ...
                              size="sm"
                              className="h-7 w-full justify-between text-xs"
                              onClick={() => setShowSemanticMappingPanel((prev) => !prev)}
                            >
                              의미 매핑(이종)
                              <span>{showSemanticMappingPanel ? "접기" : "열기"}</span>
                            </Button>
                          </div>
                          {showSemanticMappingPanel && (
                            <>
                              <div className="flex items-center justify-end">
                                <Button
                                  ...
                                  onClick={() => {
                                    handleSemanticMappingReset("A")
                                    handleSemanticMappingReset("B")
                                  }}
                                >
                                  자동값 재설정
                                </Button>
                              ...
                              <p className="text-[11px] text-muted-foreground">
                                기본값은 자동 추론되며, 필요하면 슬롯별 의미 매핑을 직접 변경할 수 있습니다.
                              </p>

                              {(["A", "B"] as UploadSlotId[]).map((slot) => {
                                const slotUi = semanticMappingUiBySlot[slot]
                                if (!slotUi) return null
                                const { columns, mapping, mode } = slotUi
                                return (
                                  <div key={`semantic-map-${slot}`} className="space-y-1.5 rounded-sm border border-border/70 p-2">
                                    <div className="flex items-center justify-between">
                                      <p className="text-[11px] font-medium">슬롯 {slot}</p>
                                      <p className="text-[11px] text-muted-foreground">정규화 모드: {mode}</p>
                                    </div>

                                    <div className="space-y-1">
                                      <Label className="text-[11px]">entity</Label>
                                      <Select
                                        value={mapping.entityColumn}
                                        onValueChange={(value) => handleSemanticMappingFieldChange(slot, "entityColumn", value)}
                                      >
                                        ...
                                        <SelectContent>
                                          {columns.map((column) => (
                                            <SelectItem key={`map-${slot}-entity-${column}`} value={column}>
                                              {column}
                                            </SelectItem>
                                          ))}
                                        </SelectContent>
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <Select
                                        ...
                                        onValueChange={(value) => handleSemanticMappingFieldChange(slot, "timeColumn", value)}
                                      >
                                        <SelectTrigger size="sm" className="h-8 text-xs">
                                          <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                          <SelectItem value={BLEND_MAPPING_HEADER_TIME_KEY}>
                                            {BLEND_MAPPING_TIME_LABEL_MAP[BLEND_MAPPING_HEADER_TIME_KEY]}
                                          </SelectItem>
                                          {columns.map((column) => (
                                            <SelectItem key={`map-${slot}-time-${column}`} value={column}>
                                              {column}
                                            </SelectItem>
                                          ))}
                                        </SelectContent>
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <Select
                                        value={mapping.metricColumn}
                                        onValueChange={(value) => handleSemanticMappingFieldChange(slot, "metricColumn", value)}
                                      >
                                        <SelectTrigger size="sm" className="h-8 text-xs">
                                          <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                          {columns.map((column) => (
                                            <SelectItem key={`map-${slot}-metric-${column}`} value={column}>
                                              {column}
                                            </SelectItem>
                                          ))}
                                        </SelectContent>
                                    ...
                                    <div className="space-y-1">
                                      ...
                                      <Select
                                        ...
                                        onValueChange={(value) => handleSemanticMappingFieldChange(slot, "valueColumn", value)}
                                      >
                                        <SelectTrigger size="sm" className="h-8 text-xs">
                                          <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                          <SelectItem value={BLEND_MAPPING_HEADER_VALUE_KEY}>
                                            {BLEND_MAPPING_VALUE_LABEL_MAP[BLEND_MAPPING_HEADER_VALUE_KEY]}
                                          </SelectItem>
                                          {columns.map((column) => (
                                            <SelectItem key={`map-${slot}-value-${column}`} value={column}>
                                              {column}
                                            </SelectItem>
                                          ))}
                                        </SelectContent>
                      ...
                      <div className="space-y-1.5 rounded-sm border border-border p-2">
                        <p className="text-xs font-medium">결합방식 선택</p>
                        {hasBothSources ? (
                          <>
                            <Select value={joinType} onValueChange={(value) => setJoinType(value as JoinType)}>
                              ...
                              <SelectContent>
                                <SelectItem value="append">시리즈 병합 (A+B)</SelectItem>
                                <SelectItem value="left">Left join (A 기준)</SelectItem>
                                <SelectItem value="inner">Inner join</SelectItem>
                                <SelectItem value="full">Full outer join</SelectItem>
                                <SelectItem value="right">Right join (B 기준)</SelectItem>
                              </SelectContent>
                            </Select>
                            <p className="text-[11px] text-muted-foreground">{JOIN_TYPE_DESCRIPTIONS[joinType]}</p>

                            {!isAppendMode && (
                              <>
                                <p className="text-xs font-medium pt-1">결합 키</p>
                                <Select
                                  value={joinKey}
                                  onValueChange={setJoinKey}
                                  disabled={joinKeyOptions.length === 0}
                                >
                                  <SelectTrigger size="sm" className="w-full h-8 text-xs">
                                    <SelectValue placeholder="결합 키 선택" />
                                  </SelectTrigger>
                                  <SelectContent>
                                    {joinKeyOptions.map((column) => (
                            ...
                            {isDualOhlcOverlayMode && (
                              <p className="text-[11px] text-muted-foreground">
                                OHLC 오버레이 모드에서는 join 방식/키 설정을 무시하고 A/B를 시간축 기준으로 함께 표시합니다.
                              </p>
                            )}
                          </>
                        ) : (
                          <p className="text-[11px] text-muted-foreground">
                            데이터 A/B가 모두 있을 때 결합 방식과 결합 키를 선택할 수 있습니다.
                          </p>
                        )}
                        {hasBothSources && !isDualOhlcOverlayMode && !isAppendMode && joinKeyOptions.length === 0 && (
                          <p className="text-[11px] text-muted-foreground">
                            공통 컬럼이 없어 결합 키 자동 제안을 제공할 수 없습니다.
                          </p>
                        )}
                      </div>

                      {joinPreview && !isAppendMode && (
                        <div className="rounded-sm border border-border p-2 text-[11px]">
                          <p className="font-medium text-foreground">join 결과 미리보기</p>
                          <div className="mt-1 space-y-0.5 text-muted-foreground">
                            <p>출력 행 수: {joinPreview.outputRows}행</p>
                            <p>A 매칭률: {joinPreview.matchedRowsA}/{joinPreview.sourceRowsA} ({formatPercent(joinPreview.matchRateA)})</p>
                            <p>B 매칭률: {joinPreview.matchedRowsB}/{joinPreview.sourceRowsB} ({formatPercent(joinPreview.matchRateB)})</p>
                          </div>
                        </div>
                      )}
                    </div>
      ...
      <Panel
        ...
        className={cn(
          "shrink-0 transition-all duration-200",
          isPanel23Open ? "w-[280px]" : "w-0 border-r-0 overflow-hidden"
        )}
      >
        {isPanel23Open && (
          <>
      ...
      {!isPanel23Open && (
        <Button
          ...
          style={{ left: isPanel1Open ? PANEL_WIDTH + 8 : 44 }}
          onClick={() => setIsConfigCollapsed(false)}
          aria-label="패널2/3 펼치기"
        >
          <ChevronsRight />
        </Button>
      )}
...
export default function ChartLabPageH() {
  return (
