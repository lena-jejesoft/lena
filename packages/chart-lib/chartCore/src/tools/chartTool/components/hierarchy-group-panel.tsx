...
import { getButtonBorderColor } from "@chartCore/src/components/ui/recharts-wrapper";
import type { HierarchyGroup } from "@chartCore/src/types/chart-config";
            ...
            <div className="space-y-1">
              {seriesFields.map((field) => {
                ...
                return (
                  <button
                    key={field}
                    className={`w-full flex items-center gap-2 px-1.5 py-1 rounded text-xs transition-colors ${isInThisGroup
                        ? "bg-accent"
                        : isInOtherGroup
                          ? "opacity-40"
                          : "hover:bg-accent/50"
