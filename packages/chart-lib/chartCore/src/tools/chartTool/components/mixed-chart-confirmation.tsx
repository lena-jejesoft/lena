...
import { Card, CardContent, CardHeader, CardTitle, CardFooter } from "@chartCore/src/components/ui/card";
...
import { parseInputDataToChartData, extractSeriesFields } from "@chartCore/src/tools/chartTool/utils/recharts-adapter";
import { getThemeColors } from "@chartCore/src/components/ui/recharts-wrapper";
...
export function MixedChartConfirmation({
  ...
  const [selectedTypeOverrides, setSelectedTypeOverrides] = useState<Record<string, "column" | "line">>({});
  const [enabledOverrides, setEnabledOverrides] = useState<Record<string, boolean>>({});

  const hasSuggestedTypes = !!suggestedYFieldTypes && Object.keys(suggestedYFieldTypes).length > 0;
  const defaultSeriesConfig = useMemo(() => {
    const defaultTypes: Record<string, "column" | "line"> = {};
    const defaultEnabled = new Set<string>();

    seriesFields.forEach((field) => {
      const suggestedType = suggestedYFieldTypes?.[field];
      if (suggestedType) {
        defaultTypes[field] = suggestedType;
        defaultEnabled.add(field);
      } else {
        defaultTypes[field] = "column";
        if (!hasSuggestedTypes) {
          defaultEnabled.add(field);
        }
      }
    });

    return { defaultTypes, defaultEnabled };
  }, [seriesFields, suggestedYFieldTypes, hasSuggestedTypes]);

  const selectedTypes = useMemo(() => {
    const merged: Record<string, "column" | "line"> = { ...defaultSeriesConfig.defaultTypes };
    seriesFields.forEach((field) => {
      const overrideType = selectedTypeOverrides[field];
      if (overrideType) {
        merged[field] = overrideType;
      }
    });
    return merged;
  }, [defaultSeriesConfig.defaultTypes, seriesFields, selectedTypeOverrides]);

  const enabledSeries = useMemo(() => {
    const merged = new Set<string>();
    seriesFields.forEach((field) => {
      const overrideEnabled = enabledOverrides[field];
      ...
      if (defaultSeriesConfig.defaultEnabled.has(field)) {
        merged.add(field);
      }
    });
    return merged;
  }, [defaultSeriesConfig.defaultEnabled, enabledOverrides, seriesFields]);
  ...
  const loading = isLoading || isSubmitting || status === "executing" || status === "inProgress";
