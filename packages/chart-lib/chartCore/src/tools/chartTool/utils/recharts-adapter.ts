import type { ChartData } from "@chartCore/src/types/sdmx";
import type { UnitSettings, DatetimeType } from "@chartCore/src/types/chart-unit-settings";
import type {
  ChartType,
  AIChartConfig,
  DataAnalysisResult,
  ...
  ChartDataItem,
  HierarchyGroup,
} from "@chartCore/src/types/chart-config";
...
export function calculateRegionHeights(
  ...
  totalHeight: number,
  minHeight: number = 30  // 물결선 보장 (zigzag 24px + 여유)
  ...
  const NORMAL_RATIO = 0.90;  // normal 영역 90%
  const OUTLIER_RATIO = 0.10; // 이상치 영역 10%
