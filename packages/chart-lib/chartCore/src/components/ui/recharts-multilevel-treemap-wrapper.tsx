...
import type { MultiLevelTreemapNode, TimepointMultiLevelTreemapData } from "@chartCore/src/tools/chartTool/utils/recharts-adapter";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "./select";
import { chartColors } from "@/lib/colors";

// 멀티레벨 트리맵 전용 색상 팔레트 (Anthropic 브랜드 스타일)
export const MULTI_LEVEL_TREEMAP_COLORS = chartColors
...
export function RechartsMultiLevelTreemapWrapper({
  ...
  const baseColors = useMemo(() => {
    ...
    if (themeColors?.seriesColors && themeColors.seriesColors.length > 0) {
