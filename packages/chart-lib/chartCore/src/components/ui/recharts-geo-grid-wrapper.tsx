...
import type { TimepointGeoGridData } from "@chartCore/src/tools/chartTool/utils/recharts-adapter";
...
export function RechartsGeoGridWrapper({
  ...
  useEffect(() => {
    ...
    onMetricLabelChange?.(label);
  }, [currentMetric, metrics, onMetricLabelChange]);
  ...
  const effectiveSeoulData = useMemo(() => {
    ...
    return data;
  }, [isTimepointMode, selectedTimepoint, timepointList, data]);
  ...
  const effectiveNationalData = useMemo(() => {
    ...
    return nationalData;
  }, [isTimepointMode, selectedTimepoint, timepointList, nationalData]);
        ...
        <div>
          <div className="flex items-center gap-1.5 text-xs">
            ...
            <button
              ...
              className={`relative w-6 h-3.5 rounded-full transition-colors ${mapLevel === "national" ? "bg-muted-foreground" : "bg-muted-foreground/30"
                }`}
            >
              <div className={`absolute top-0.5 w-2.5 h-2.5 bg-white rounded-full shadow transition-transform ${mapLevel === "national" ? "translate-x-[12px]" : "translate-x-0.5"
                }`} />
            </button>
