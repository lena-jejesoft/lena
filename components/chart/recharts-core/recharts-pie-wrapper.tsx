...
export function RechartsPieWrapper({
  ...
  const isSmallMultiples = timepointData && timepointData.length > 0;
  ...
  useEffect(() => {
    if (isSmallMultiples && selectedTimepoint) {
      ...
      if (onSelectedDataChange && timepointData) {
        const selectedData = timepointData.find(tp => tp.timepoint === selectedTimepoint);
        if (selectedData) {
          onSelectedDataChange(selectedData.data);
        }
      }
    }
  }, [isSmallMultiples, selectedTimepoint, onTooltipChange, onSelectedDataChange, timepointData]);

  if (isSmallMultiples) {
    const count = timepointData.length;
