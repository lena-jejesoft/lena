...
import type { ChartThemeColors, ChartType, YAxisPlacement } from "./recharts-wrapper";
import { generateNiceTicks } from "./recharts-wrapper";
...
import { ChartDataItem, OutlierInfo } from "./recharts-split-types";
...
export function RechartsRegionChart({
  ...
  const chartData = baseData.map((item) => {
    ...
    // 해당 날짜의 이상치들 가져오기
    const dateOutliers = outliersByDate.get(item.date_display as string) || [];
  ...
  const colors = colorOverride && colorOverride.length > 0
    ? colorOverride
  ...
  const renderSeries = (fieldsToRender?: string[]) => {
    ...
    renderFields.forEach((field) => {
      ...
      const color = originalIndex >= 0
        ? colors[originalIndex % colors.length] || colors[0]
        : colors[0];
