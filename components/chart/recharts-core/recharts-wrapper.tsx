...
/** 지원하는 차트 타입 */
export type ChartType = "column" | "line" | "area" | "area-100" | "stacked-area" | "synced-area" | "mixed" | "stacked" | "stacked-100" | "stacked-grouped" | "dual-axis" | "dual-axis-stacked-bar" | "pie" | "two-level-pie" | "treemap" | "multi-level-treemap" | "ranking-bar" | "geo-grid" | "regression-scatter" | "radar";
/** Y축 배치 타입 */
export type YAxisPlacement = "left" | "right";

// import { formatDateForXAxis } from "@/tools/chartTool/utils/recharts-adapter";
...
/** Light 모드 fallback 색상 (Shadow DOM에서 사용) */
...
/** 테마 설정 */
export function setChartTheme(theme: 'light' | 'dark') {
  currentTheme = theme;
}
...
function getCSSVariable(varName: string): string {
  const fallbackColors = currentTheme === 'dark' ? DARK_FALLBACK_COLORS : LIGHT_FALLBACK_COLORS;

  if (typeof window === "undefined") return fallbackColors[varName] || "";
  const root = document.documentElement;
  const value = getComputedStyle(root).getPropertyValue(varName).trim();
  // fallback for Shadow DOM context where CSS variables aren't accessible
  if (!value && fallbackColors[varName]) {
    return fallbackColors[varName];
  }
  // 이미 hsl() 형식이면 그대로 반환
 ...
 * 라이트 모드: hsl(0 0% 44%), 다크 모드: hsl(0 0% 70%)
 */
export function getAxisLineColor(): string {
  // currentTheme 변수 사용 (Shadow DOM에서도 정확하게 동작)
  return currentTheme === 'dark' ? "hsl(0 0% 70%)" : "hsl(0 0% 44%)";
}
...
export function RechartsWrapper({
  ...
  const baseSeriesColors = useMemo(() => {
    if (colorOverride && colorOverride.length > 0) {
      return colorOverride;
    }
    return themeColors?.seriesColors || [
      "hsl(var(--chart-1))",
      "hsl(var(--chart-2))",
      "hsl(var(--chart-3))",
      "hsl(var(--chart-4))",
  ...
  }, [colorOverride, themeColors?.seriesColors]);

  const colors = useMemo(
    () => expandSeriesColors(baseSeriesColors, yFields.length),
    [baseSeriesColors, yFields.length]
  );
  ...
  const getColorForField = useCallback((field: string) => {
    ...
    return originalIndex >= 0
      ? baseSeriesColors[originalIndex % baseSeriesColors.length] || colors[0]
      : colors[0];
  }, [allSeriesFields, yFields, baseSeriesColors, colors]);
  ...
  const zeroLineStyleForNormalChart = useMemo(() => {
    ...
    return style;
  }, [data, yFields, transformDataForMixedSeries, chartType, isStackedGroupedLike, yFieldTypes]);
  ...
  return (
    <div ref={chartContainerRef} style={{ width: '100%', height, position: 'relative' }}>
      {shouldSplitStack && splitStackData && splitDomains ? (
        ...
        <YAxis
          ...
          domain={(chartType === "stacked" || chartType === "stacked-100" || isStackedGroupedLike || chartType === "area-100") && transformDataForMixedSeries.yAxisDomain ? transformDataForMixedSeries.yAxisDomain : undefined}
    ...
    {/* Brush in separate container - margins match chart area */}
    {showBrush && (() => {
      const brushColors = getBrushColors();
      // YAxis width: dual-axis uses explicit 50px left axis, others use default ~60px
      const leftMargin = isDualAxisLike ? 50 : 60;
      const rightMargin = isDualAxisLike ? 10 : 30;

      return (
        <div style={{ position: 'relative', height: BRUSH_HEIGHT, marginTop: BRUSH_MARGIN_TOP, marginLeft: leftMargin, marginRight: rightMargin }}>
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
              <XAxis dataKey={xField} hide />
              <Brush
    ...
    {shouldUseShade && hoveredCategoryBounds && (() => {
      ...
      const shadeHeight = effectiveHeight - 10 - X_AXIS_HEIGHT;
